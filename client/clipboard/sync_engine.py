...
class SyncEngine:
    ...
    async def sync_to_peer(self, peer_id: str, clipboard_content: str, device_id: str, device_name: str) -> bool:
        ...
        """
        try:
            logger.info(f"[SYNC] === Starting sync to {peer_id[:8]}... ===")
            logger.info(f"[SYNC] Content length: {len(clipboard_content)} chars")
            
            # Check if peer is online and connected
            is_connected = self.webrtc_manager.is_connected(peer_id)
            logger.info(f"[SYNC] WebRTC connected: {is_connected}")
            
            if not is_connected:
                logger.warning(f"[SYNC] Peer {peer_id[:8]} not connected, queuing for later")
                await self.queue_sync_if_offline(peer_id, clipboard_content)
                return False

            # Check if data channel is open
            channel_open = self.data_channel_handler.is_channel_open(peer_id)
            logger.info(f"[SYNC] Data channel open: {channel_open}")
            
            if not channel_open:
                logger.warning(f"[SYNC] Data channel to {peer_id[:8]} not open, queuing")
                await self.queue_sync_if_offline(peer_id, clipboard_content)
                return False
            ...
            logger.info(f"[SYNC] Metadata prepared: seq={metadata['sequence_number']}")

            # Send via data channel
            logger.info(f"[SYNC] Sending via data channel...")
            ...
            if success:
                ...
                logger.info(f"[SYNC] ✓ Successfully synced to {peer_id[:8]}: {len(clipboard_content)} chars")

                if self.on_sync_callback:
                    await self.on_sync_callback(peer_id, 'sent', len(clipboard_content))
            else:
                logger.error(f"[SYNC] ✗ Failed to send to {peer_id[:8]}")

            return success

        except Exception as e:
            logger.error(f"[SYNC] ✗ Error syncing to {peer_id[:8]}: {e}", exc_info=True)
            return False
    ...
    async def receive_from_peer(self, peer_id: str, content: str, timestamp: float,
        ...
        """
        try:
            logger.info(f"[RECV] === Received clipboard from {device_name} ({peer_id[:8]}...) ===")
            logger.info(f"[RECV] Content length: {len(content)} chars")

            # Check if we should update local clipboard
            should_update = self.should_update_local_clipboard(content, peer_id)
            logger.info(f"[RECV] Should update local clipboard: {should_update}")
            
            if should_update:
                # Update local clipboard
                logger.info(f"[RECV] Setting local clipboard...")
                success = clipboard_monitor.set_clipboard(content)

                if success:
                    self.last_sync_time[peer_id] = time.time()
                    logger.info(f"[RECV] ✓ Updated local clipboard from {device_name}")
                    ...
                    logger.error(f"[RECV] ✗ Failed to update local clipboard")
                    return False
            else:
                logger.debug("[RECV] Skipping clipboard update (filtered)")
                return False

        except Exception as e:
            logger.error(f"[RECV] ✗ Error receiving from peer {peer_id[:8]}: {e}", exc_info=True)
            return False
