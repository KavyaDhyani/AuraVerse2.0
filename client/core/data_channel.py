...
class DataChannelHandler:
    ...
    async def send_clipboard_data(self, peer_id: str, content: str, metadata: dict):
        ...
        try:
            ...
            if not channel:
                logger.warning(f"[SEND] No data channel available for peer: {peer_id[:8]}...")
                return False

            state = channel.readyState
            if state != 'open':
                logger.warning(f"[SEND] Data channel not open for peer: {peer_id[:8]}... (state: {state})")
                return False
            ...
            logger.info(f"[SEND] Sending {len(content)} chars to {peer_id[:8]}...")
            channel.send(message_str)

            logger.info(f"[SEND] ✓ Sent clipboard data to {peer_id[:8]}: {len(content)} chars")
            return True

        except Exception as e:
            logger.error(f"[SEND] ✗ Error sending clipboard data to {peer_id[:8]}: {e}", exc_info=True)
            return False
